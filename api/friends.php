...
if ("GET_FRIENDS_BY_CONTACT" == $action) {
  ...
  if ($result = $conn->query($sql)) {
    if ($result->num_rows > 0) {
      while ($row = $result->fetch_assoc()) {
        $friend2 = $row['friend2'];
        $sql_users = $conn->query("SELECT `name`, `phone`, `created_at` FROM invites WHERE `id` = '$friend2'");
...
if ("GET_FRIENDS_BY_INVITE" == $action) {
  $status = $_POST['status'];
...
if ("GET_FRIENDS_BY_PERIOD" == $action) {
